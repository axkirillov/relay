...
const { file, open, read, stale, tilde } = await import("./plan.ts");
const { plansDir } = await import("./paths.ts");
...
// uses — which is what keeps `relay --plan` and `composer --plan` on one file while
// both flags exist.
check("the plan is a file under the plans directory", file(repo), join(plansDir(), `${basename(taskDir(repo))}.md`));
check("named for the task the ledger names", basename(file(repo)), `${basename(taskDir(repo))}.md`);
check("nothing exists until it is asked for", exists(plansDir()), false);
check("asking for the path makes somewhere to write it", open(repo), file(repo));
check("and the directory is there", exists(plansDir()), true);
